...
if (!empty($_GET['search_word'])) {
    ...
    $search_query   .= 'AND (faq_title LIKE "%'.$search_word.'%")';
    $filters        .= '&search_word='.$search_word.'';
}

$search_word = (isset($_GET['search']) ? $_GET['search_word'] : '');
$condition = array(
                     'select'       =>  'faq_id, faq_status, faq_title'
                    ,'where'        =>  array(
                                                'is_deleted'  => 0
                                            )
...
<div class="card">
    ...
    <div class="card-body">   
        ...
        $condition['order_by'] = "faq_id ASC LIMIT " . ($page - 1) * $Limit . ",$Limit";
        $condition['return_type'] = 'all';

        $regions     = $dblms->getRows(FAQS, $condition);
        if ($regions) {
            echo'
                    ...
                    <tbody>';
                        ...
                        foreach ($regions as $row) {
                            ...
                            <tr style="vertical-align: middle;">
                                ...
                                <td class="text-center">'.$row['faq_title'].'</td>
                                <td class="text-center">'.get_status($row['faq_status']).'</td>
                                <td class="text-center">
                                    <div class="dropdown">
                                        <button class="btn btn-soft-primary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="ri-more-fill"></i></button>
                                        <ul class="dropdown-menu dropdown-menu-end" style="cursor: pointer;">
                                            <li><a href="?id='.$row['faq_id'].'" class="dropdown-item"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>
                                            <li><a class="dropdown-item" onclick="confirm_modal(\''.moduleName().'.php?deleteid='.$row['faq_id'].'\');"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>';
